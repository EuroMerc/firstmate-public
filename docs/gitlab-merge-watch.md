...
The live registration tag is `fm-pr-poll-registration-v3`, which includes the provider tag plus the validated phase head, publisher kind, transition epoch, and observation fingerprint.
A `fm-pr-poll-registration-v1` or `fm-pr-poll-registration-v2` record no longer parses.
Arm a current watch with `bin/fm-pr-check.sh`; a silent same-PR re-arm preserves the phase fields until a structured observation proves a different forge head.
