...
| `fm-external-wait-lib.sh` | Render and transition visible worker and PR external waits in Europe/Berlin          |
...
| `fm-pr-check.sh`         | Record validated `pr=` and `pr_head=` values, atomically arm a static merge poll, and optionally observe a newly opened PR once; refuses a GitHub draft |
