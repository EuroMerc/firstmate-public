...
Only the crew's own declaration qualifies: every pause-cadence gate reads the newest status event that firstmate did not write itself, skipping the PR-wait presentation `bin/fm-external-wait-lib.sh` appends to the same log, which it recognizes by its exact published form and a canonical PR URL that re-parses to itself rather than by any label prefix, so the recognition also holds after the poll is retired.
A firstmate-published PR wait is therefore presentation only and can never suppress wedge detection or a liveness check for a live crew.
...
Live or inconclusive liveness remains fail-open at that initial surface, which an idle pane merely changing its render does not reopen, and a secondmate's endpoint liveness is still never read at all; a mate is admitted to that same cadence only to serve a declared wait's bounded re-surface, so a forgotten pause or captain hold on a mate cannot rot invisibly.
...
In that status-log fallback, a declared external wait reports the distinct `paused` state through `bin/fm-external-wait-lib.sh`'s persistent presentation, with its reason, Europe/Berlin start time derived from the status log mtime, and explicit healthy-worker label.
A validated pending PR observation uses the same declared-wait event and presentation, names concrete pending checks when the forge supplies them, and stores its transition epoch, normalized fingerprint, and validated forge head in the existing identity-bound PR-poll registration; only the first phase falls back to registration time.
...
That grace is one-way per head: it opens from an unbound phase or a proven different head only, so once the same head has resolved to no-checks - or has already reported any other phase - a later empty rollup stays no-checks instead of reopening the grace and oscillating between a wait and a no-checks result.
A GitHub check that is only waiting on a human approval gate (`ACTION_REQUIRED`) counts as pending like GitLab's `manual`, while a check that produced no result for the current ref (`STALE`) stays truthfully actionable.
A rollup member of an unrecognized type can never claim green: it reports unreadable unless a readable member is already red or still running.
GitLab's richer observation reads merge-request state and head-pipeline status from the existing structured response: `open`, `opened`, and the transient `locked` state GitLab sets while it processes a merge all classify through the head pipeline, every known non-terminal pipeline status including `manual` and `canceling` remains pending, and only a genuinely unknown value is unreadable.
A skipped or absent GitLab pipeline is no running check, but firstmate's own merge path refuses that merge request, so it is published as an actionable not-merge-ready state rather than delivery readiness and stays under observation.
...
A presentation failure is always best effort: it is recorded for triage and never preempts the merge notification, the truthful closure, or the identity-bound poll retirement.
A pending phase that follows one of this publisher's own non-pending states starts from that transition, while a start already published for a running phase stays stable.
An unreadable observation is a failed read of the same check run rather than the end of a wait, so for the same head the interrupted phase survives it whole: the published start is kept, and an empty-rollup startup grace keeps both its epoch and its remaining time instead of resolving early.
A silent re-arm preserves the registration-bound phase: the first valid head binds an unknown phase without resetting it, the same validated head keeps its start, and only a structured observation proving a different head or pending after another publisher state begins a new phase.
The presentation owner bounds only the free worker-authored reason, so the canonical PR URL, Europe/Berlin timestamp, and healthy-worker label always stay complete; worker prose that merely imitates the PR wait label fails the publisher's structural check and is bounded as the worker's own wait.
Deduplication compares an observation with the normalized fingerprint in that registration-bound phase record, so an unrelated worker event in between and a silent same-head re-arm both keep an unchanged wait silent and keep its published start.
Only a declared wait reported from the status-log owner keeps its full detail in a main-home bearings row; prose from any other state stays under the ordinary cap.
The bearings projection preserves the complete bounded external-wait reason, canonical PR URL, timestamp, and healthy-worker label for both main-home tasks and the narrowly typed external-wait detail in a secondmate home summary.
That secondmate field is admitted only for a reconciled `paused` state from `status-log`; `parked`, `blocked`, decision, and working prose cannot gain typed wait provenance.
Ordinary detail caps remain for every unrelated row, a secondmate row keeps its first three waits complete with the remainder counted, and the home's remaining non-wait holds keep the same shape beside them: the first three under the ordinary per-hold cap with their own remainder count, so one row cannot grow without bound in either list.
