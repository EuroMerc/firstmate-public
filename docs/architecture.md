...
A validated pending PR observation uses the same declared-wait event and presentation, derives its start time from the existing PR-poll registration mtime, and names concrete pending checks when the forge supplies them.
...
An unreadable observation is a failed read of the same check run rather than the end of a wait, so the published start survives it; only another publisher state or a registration re-armed for a later head begins a new phase.
...
Deduplication compares an observation against the most recent event this publisher itself wrote for the same canonical PR, so an unrelated worker event in between and a silent re-arm both keep an unchanged wait silent and keep its published start.
...
The bearings projection preserves the complete bounded external-wait reason, canonical PR URL, timestamp, and healthy-worker label for both main-home tasks and the narrowly typed external-wait detail in a secondmate home summary, while retaining its ordinary detail caps for every unrelated row, keeping a secondmate row's first three waits complete with the remainder counted, and still listing that home's remaining non-wait holds under those caps.
