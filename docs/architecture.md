...
GitLab's richer observation reads merge-request state and head-pipeline status from the existing structured response: `open` and `opened` are tolerated, every known non-terminal pipeline status including `manual` and `canceling` remains pending, and only a genuinely unknown value is unreadable.
...
An unreadable observation is a failed read of the same check run rather than the end of a wait, so the published start survives it.
