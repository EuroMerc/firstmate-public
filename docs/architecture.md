...
Registration publishes only a real pending sample; repeated monitoring owns red and unreadable escalation, while merge appends the existing terminal status transition before poll retirement so no pending wait remains current.
