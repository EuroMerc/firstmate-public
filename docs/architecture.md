...
GitLab's richer observation reads merge-request state and head-pipeline status from the existing structured response: `open` and `opened` are tolerated, every known non-terminal pipeline status including `manual` and `canceling` remains pending, a skipped or absent pipeline means no running checks, and only a genuinely unknown value is unreadable.
