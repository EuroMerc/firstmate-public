...
The launch path is therefore a supervisor-backend decision rather than a harness one: a harness-native in-pane background job is only available where the backend reports no native agent state.
...
For every other harness under Herdr, `bin/fm-afk-launch.sh` creates a dedicated unfocused Herdr workspace, runs the daemon there with an explicit supervisor target and backend, records the exact daemon pane, and closes only that pane on stop.
