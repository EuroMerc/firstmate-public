...
# merged change and stay silent otherwise, including on every error, so a failed
# lookup can never be read as a merge.
...
# For GitHub and GitLab both forms emit one structural observation: merged,
# closed, green, empty, no-pipeline, unreadable, pending, or failed, plus a
# sanitized qualifier after one tab when the forge supplies one (the diagnostic
# form keeps that qualifier only for pending and failed). `empty` is a GitHub
# rollup whose startup grace is decided from the existing registration
# timestamp by bin/fm-external-wait-lib.sh; `no-pipeline` is a GitLab response
# whose head pipeline is absent or `skipped` (the qualifier names which), and
# firstmate's own merge path refuses both, so it is reported as an actionable
# not-merge-ready state rather than delivery readiness. A rollup member of an
# unrecognized type can never claim green: it reports unreadable unless a
# readable member is already red or still running. Gerrit keeps the merged-only
# contract in every form.
...
# tooling to follow the first two. The Gerrit branch and the GitLab observation
# additionally need jq, which bin/fm-pr-check.sh refuses to arm a Gerrit watch
# without.
set -u
LC_ALL=C
export LC_ALL
