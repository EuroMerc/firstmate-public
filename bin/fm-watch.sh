...
wedge_wait_evidence() {  # <task> -> one wait_record on stdout
  ...
  last=$(fm_external_wait_worker_last_status_line "$statusf")
...
busy_turn_bound_check() {  # <window> <task> <hash> <since-file> <escalation-file>
  ...
  statusf="$STATE/$task.status"
  if status_is_paused_or_captain_held "$(fm_external_wait_worker_last_status_line "$statusf")"; then
    if afk_present; then
      ...
      if captain_held_silenced "$(fm_external_wait_worker_last_status_line "$statusf")"; then
