...
# head, kind, epoch, and fingerprint. Version v3 added those phase fields, so a
# registration written by an earlier release is recognised as old and refused.
...
fm_pr_poll_registration_rerecord_device() {  # <state> <id> <template>
  ...
  local phase_head phase_kind phase_epoch phase_signature
  ...
  if ! printf '%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n' \
      fm-pr-poll-registration-v3 "$id_line" "$provider" "$url" "$host" "$path" "$number" \
