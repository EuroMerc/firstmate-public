...
2. **Ensure the sub-supervisor daemon is running as a tracked background process.**
   Its hosting depends on the harness AND on the supervisor backend, and the backend has the stronger claim.
   Pick the right path:
   - **Supervisor backend that reports native agent state (Herdr), or a harness with no native in-pane background tool (e.g. pi): run `bin/fm-afk-launch.sh start`.**
   ...
   - **Harness WITH a native in-pane tracked-background tool** (e.g. claude's background bash, grok's background tool) **on a backend that reports no native agent state**: first run `bin/fm-afk-launch.sh start-native`, then run `FM_AFK_STATE_PREPARED=1 bin/fm-afk-start.sh` through that native tool.
   ...
   It reports an already-running daemon only after revalidating the identity-backed lock; if that holder disappears during the refresh, it follows the ordinary no-live refusal or recovery path instead.
