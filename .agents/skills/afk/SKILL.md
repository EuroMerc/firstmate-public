...
2. **Per harness, after the record exists:**
   ...
   - **Any other harness on a supervisor backend that reports native agent state (Herdr)**: run `bin/fm-afk-launch.sh start`, whatever in-pane background tool the harness has.
     The backend decides here rather than the harness, for the reason owned by docs/herdr-backend.md "Away-mode supervisor support".
   ...
   - **Harness WITH a native in-pane tracked-background tool** (claude's background bash without the supervision host, grok's background tool) **on a backend that reports no native agent state**: run `bin/fm-afk-launch.sh start-native`, then run `FM_AFK_STATE_PREPARED=1 bin/fm-afk-start.sh` through that native tool.
