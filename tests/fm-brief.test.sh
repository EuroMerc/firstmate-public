...
test_faster_paths_use_configured_authority_without_stacked_review() {
  ...
  assert_grep 'do not wait for them before reporting done' "$brief" \
    "direct-PR readiness regressed from PR-open to checks-green"
  # shellcheck disable=SC2016 # Literal backticks and braces are generated brief prose.
  assert_grep 'Then append `done [at=<epoch>]: PR {url}`' "$brief" \
    "direct-PR readiness no longer reports done once the PR is open"
