...
#
# The reply window is anchored to the run's own clock rather than a literal
# calendar date, so the seeded thread is always reachable no matter when the
# suite runs; tests that need a past or closing window pin it with
# FMX_NOW_OVERRIDE against REPRO_EXPIRES_EPOCH.
pf_iso_at() {  # <epoch-seconds>
  date -u -r "$1" +%Y-%m-%dT%H:%M:%SZ 2>/dev/null \
    || date -u -d "@$1" +%Y-%m-%dT%H:%M:%SZ
}
REPRO_EXPIRES_EPOCH=$(( $(date -u +%s) + 604800 ))
REPRO_EXPIRES_AT=$(pf_iso_at "$REPRO_EXPIRES_EPOCH")
REPRO_RECEIVED_AT=$(pf_iso_at $((REPRO_EXPIRES_EPOCH - 604800)))

seed_repro_commitment() {   # <home> <obligation> <request> <work-home> <work-id>
  local home=$1 obligation=$2 request=$3 work_home=$4 work_id=$5
  jq -n --arg r "$request" --arg rec "$REPRO_RECEIVED_AT" --arg exp "$REPRO_EXPIRES_AT" \
