...
Run `bin/fm-pr-check.sh --observe-pending <id> <PR url>` with the URL copied from that ready signal or the resolved checks-green `fm-crew-state.sh` line - it records `pr=` and the forge's `pr_head=` when available in the task's meta, arms the watcher's merge poll, and takes the one immediate non-watching observation reserved for a newly opened ready PR; later re-arms omit `--observe-pending`.
