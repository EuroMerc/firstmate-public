...
Run `bin/fm-pr-check.sh --observe-pending <id> <PR url>` - it records `pr=` and the forge's `pr_head=` when available in the task's meta, arms the watcher's merge poll, and takes the one immediate non-watching observation reserved for a newly opened ready PR; later re-arms omit `--observe-pending`.
